...
export default function RemindersGrid() {
  ...
  const viewRemindersResponseData = useSelector((state) => {
    return state.reminder.viewRemindersResponseData;
  });

  const isViewRemindersResponseReceived = useSelector((state) => {
    return state.reminder.isViewRemindersResponseReceived;
  });

  const prevIsViewRemindersResponseReceived = usePrevious(
    isViewRemindersResponseReceived
  );

  useEffect(() => {
    if (
      prevIsViewRemindersResponseReceived !== undefined &&
      prevIsViewRemindersResponseReceived !== isViewRemindersResponseReceived
    ) {
      if (isSuccessfulResponse(viewRemindersResponseData)) {
        // showPopup("success", "Success", "Payment Reminder Successfully fetched");
        setRemindersList(
          viewRemindersResponseData.success
            .filter((reminder) => new Date(reminder.date) > new Date())
            .map((reminder) => {
              let dateFormatted = reminder;
              dateFormatted.formattedDate = moment(
  ...
  const deleteReminderResponseData = useSelector((state) => {
    return state.reminder.deleteReminderResponseData;
  });

  const isDeleteReminderResponseReceived = useSelector((state) => {
    return state.reminder.isDeleteReminderResponseReceived;
  });

  const prevIsDeleteReminderResponseReceived = usePrevious(
    isDeleteReminderResponseReceived
  );

  useEffect(() => {
    if (
      prevIsDeleteReminderResponseReceived !== undefined &&
      prevIsDeleteReminderResponseReceived !== isDeleteReminderResponseReceived
    ) {
      if (isSuccessfulResponse(deleteReminderResponseData)) {
        showPopup(
          "success",
          "Success",
          "Payment Reminder Successfully Deleted!"
        );
  ...
  const editReminderResponseData = useSelector((state) => {
    return state.reminder.editReminderResponseData;
  });

  const isEditReminderResponseReceived = useSelector((state) => {
    return state.reminder.isEditReminderResponseReceived;
  });

  const prevIsEditReminderResponseReceived = usePrevious(
    isEditReminderResponseReceived
  );

  useEffect(() => {
    if (
      prevIsEditReminderResponseReceived !== undefined &&
      prevIsEditReminderResponseReceived !== isEditReminderResponseReceived
    ) {
      if (isSuccessfulResponse(editReminderResponseData)) {
        showPopup(
          "success",
          "Success",
          "Payment Reminder Successfully Modified!"
        );
        window.location.reload();
      }
    }
  ...
  const handleDeleteClose = () => {
    setShow(false);
    setSnackbar({
      message: "Payment Reminder Successfully Deleted!",
      severity: "success",
      visibility: true,
    });
  };
  ...
  function handleEditSuccess() {
    ...
    setSnackbar({
      message: "Payment Reminder Successfully Modified!",
      severity: "success",
      visibility: true,
    });
  }
    ...
    <Container fluid>
      <Row className=" mt-3 text-black container-fluid justify-content-around">
        <Button
          className="rounded-3"
          style={{ width: "250px" }}
          onClick={() => history.push(routes.createReminder.path)}
        >
          Create Reminder
        </Button>
      </Row>
      <Row className="text-center" hidden={remindersList.length !== 0}>
        <label>
          {" "}
          No reminders found, please create a new Payment Reminder!{" "}
        </label>
      </Row>
            ...
            >
              <Row>
                <Col md={3} className="d-none d-md-block pt-4 ">
                  <img className="w-75" alt="Reminder Icon" src={StickyNote} />
                </Col>
                <Col className="p-3">
                  <Stack gap={1}>
      ...
      <Modal show={show}>
        ...
        <Modal.Body>
          Are you sure you want to delete payment reminder for{" "}
      ...
      <Snackbar
        ...
        onClose={() =>
          setSnackbar({ message: "", severity: "success", visibility: false })
        }
      >
        <Alert
          onClose={() =>
